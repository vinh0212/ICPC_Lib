...
int main()
{
    ...
    if ((lb != -1) && (ub != -1))
    {
        cout << "P = '" << P << "' is found SA[" << lb << ".." << ub << "] of T = '" << T << "'\n";
        cout << "They are:\n";
        for (int i = lb; i <= ub; ++i)
            cout << "  " << (T + S.SA[i]) << "\n";
    }
    ...
    LRS_ans[LRS_len] = '\0'; // null-terminate the string
    cout << "The LRS is '" << LRS_ans << "' with length = " << LRS_len << "\n";
    ...
    int split_idx = n - m - 1;
    cout << "T+P = '" << T << "'\n";
    cout << " i SA[i] LCP[i] From  Suffix SA[i]\n";
    for (int i = 0; i < n; ++i)
        cout << setw(2) << i << "    " 
             << setw(2) << S2.SA[i] << "    " 
             << setw(2) << S2.LCP[i] << "    " 
             << (S2.SA[i] < split_idx ? 1 : 2) << "    " 
             << (T + S2.SA[i]) << "\n";

    auto [LCS_len, LCS_idx] = S2.LCS(split_idx);
    strncpy(LCS_ans, T + S2.SA[LCS_idx], LCS_len);
    LCS_ans[LCS_len] = '\0'; // null-terminate the string
    cout << "The LCS is '" << LCS_ans << "' with length = " << LCS_len << "\n";
